...
const AppShell = () => {
  ...
  const resumeBuilderNavPath = !user
    ? '/login'
    : (user.role === 'student' || user.role === 'admin')
      ? '/portal/student/profile?section=resume&focus=resume-builder#resume-builder'
      : user.role === 'retired_employee'
        ? '/portal/student/profile?section=resume&focus=resume-builder#resume-builder'
      : '/login';
  ...
  const availableLinks = [
    ...
    { key: 'jobs', to: jobsNavPath, label: 'Jobs' },
    { key: 'ats', to: '/ats', label: 'ATS' },
    { key: 'resume-builder', to: resumeBuilderNavPath, label: 'Resume Builder' },
  ...
  const isPortalWorkbench = /^\/portal\/(admin|hr|student|platform|audit|dataentry|accounts|sales|support|super-admin)\b/i.test(location.pathname);
  ...
  const employerMenuItems = [
    ...(dashboardPath ? [{
      label: 'Dashboard',
      to: dashboardPath,
      hint: 'Open your assigned workspace'
    }] : []),
    {
    ...
    {
      label: 'Post a Job for Retired Employee',
      to: user && (user.role === 'hr' || user.role === 'admin') ? '/portal/hr/jobs?audience=retired_employee' : '/sign-up',
      hint: 'Publish openings dedicated to retired professionals'
    },
    {
      ...
      <header className={`site-header ${isPortalWorkbench ? 'site-header--portal' : ''}`}>
        <div className="site-header-inner">
          <Link to="/" className="site-brand">
            ...
            <span className="site-brand-copy">HHH Jobs</span>
          </Link>
            ...
            <div className="site-nav-mobile-actions">
              ...
              <div className={`site-employer-menu site-employer-menu--mobile ${isEmployerMenuOpen ? 'is-open' : ''}`} ref={employerMenuMobileRef}>
                ...
                <div className="site-employer-dropdown" role="menu" aria-label="Employer options">
                  <div className="site-employer-dropdown-head">
                    <p>Employer Actions</p>
                    <span>Choose a hiring path</span>
                  </div>
                  {employerMenuItems.map((item) => (
                    <Link
                      key={`mobile-${item.label}`}
                      to={item.to}
                      className={`site-employer-item ${item.label === 'Post a Job for Retired Employee' ? 'site-employer-item--featured' : ''}`}
                      onClick={() => {
                        setIsEmployerMenuOpen(false);
                        setIsMenuOpen(false);
                      }}
