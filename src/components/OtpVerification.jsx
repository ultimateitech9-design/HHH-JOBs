    ...
    const handleVerify = async () => {
        ...
        try {
            ...
            try {
                ...
                Swal.fire({ icon: 'error', title: 'Error', text: fallbackError.message || 'Something went wrong. Please try again.' });
            }
        } finally {
        ...
        try {
            ...
            try {
                ...
                if (nextOtp.length === 6) {
                    setOtp(nextOtp.split(''));
                    inputRefs.current[5]?.focus();
                    Swal.fire({ icon: 'success', title: 'OTP Sent', text: 'A new OTP has been generated for local testing.' });
                    return;
                }
            } catch (fallbackError) {
                Swal.fire({ icon: 'error', title: 'Error', text: fallbackError.message || 'Network error. Please try again.' });
            }
        }
    };
