...
const UsersManagement = () => {
  ...
  const cards = useMemo(() => [
    ...
    { label: 'Blocked / Suspended', value: String(users.filter((item) => ['blocked', 'suspended'].includes(item.status)).length), helper: 'Trust and policy watchlist', tone: 'danger' },
  ...
  const handleSuspend = async () => {
    ...
    const updated = await updateUserStatus(targetUser.id, targetUser.status === 'active' ? 'suspended' : 'active');
    setUsers((current) => current.map((user) => (user.id === targetUser.id ? { ...user, ...updated } : user)));
    setTargetUser(null);
  };
  ...
  return (
    <div className="module-page module-page--admin">
      ...
      <section className="panel-card">
        <FilterBar
          ...
          fields={[
            ...
            { key: 'status', label: 'Status', options: ['active', 'pending', 'blocked', 'suspended'].map((status) => ({ value: status, label: status })) }
          ]}
          actions={(
            <>
              <button
      ...
      <ConfirmModal
        ...
        message={targetUser ? `Set ${targetUser.name} to ${targetUser.status === 'active' ? 'suspended' : 'active'}?` : ''}
