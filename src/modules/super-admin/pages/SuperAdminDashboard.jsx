...
const SuperAdminDashboard = () => {
  const { dashboard, loading, error, isDemo } = useDashboardStats();
  ...
  const workspaceSnapshots = useMemo(() => {
    ...
    const users = source.users?.length ? source.users : adminDummyData.users;
    const supportTickets = source.supportTickets?.length ? source.supportTickets : adminDummyData.supportTickets;
    const payments = source.payments?.length ? source.payments : adminDummyData.payments;
    const jobs = source.jobs?.length ? source.jobs : adminDummyData.jobs;
    const applications = source.applications?.length ? source.applications : adminDummyData.applications;
    const logs = source.systemLogs?.length ? source.systemLogs : adminDummyData.systemLogs;
    const stats = source.stats || adminDummyData.dashboardStats;
  ...
  return (
    <div className="module-page module-page--admin">
      <AdminHeader title="Super Admin Dashboard" subtitle="Command center for platform health, governance, billing risk, operational workload, and trust signals across HHH Jobs." />
      {isDemo ? <p className="module-note">Demo super admin data is shown because backend endpoints are not connected yet.</p> : null}
      {error ? <p className="form-error">{error}</p> : null}

      <section className="dash-banner dash-banner--admin">
        <div>
          <p className="dash-banner-kicker">Platform Command Center</p>
          <h2>One workspace to monitor growth, risk, revenue, trust, and operations</h2>
          <p>Super admin can control users, employers, jobs, applications, payments, subscriptions, support load, permissions, and system configuration from this module.</p>
          <div className="dash-banner-actions">
            <Link to="/portal/super-admin/users" className="btn-primary">Open users control</Link>
            <Link to="/portal/super-admin/system-settings" className="btn-secondary">Review system settings</Link>
          </div>
        </div>
