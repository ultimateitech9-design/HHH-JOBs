...
const JobsManagement = () => {
  ...
  const cards = useMemo(() => [
    ...
    { label: 'Suspended', value: String(jobs.filter((job) => job.status === 'suspended').length), helper: 'Policy enforcement state', tone: 'danger' },
  ...
  const handleToggleStatus = async () => {
    if (!targetJob) return;
    const nextStatus = targetJob.status === 'open' ? 'suspended' : 'open';
    ...
    <div className="module-page module-page--admin">
      ...
      <section className="panel-card">
        <FilterBar
          ...
          fields={[{ key: 'status', label: 'Status', options: ['open', 'pending', 'closed', 'suspended', 'approved', 'rejected'].map((status) => ({ value: status, label: status })) }]}
      ...
      <ConfirmModal
        ...
        message={targetJob ? `Set ${targetJob.title} to ${targetJob.status === 'open' ? 'suspended' : 'open'}?` : ''}
