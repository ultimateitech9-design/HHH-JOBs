...
//     value: 'classic',
//     label: 'Classic',
//     subtitle: 'ATS-friendly structure',
//     description: 'Balanced sections with traditional professional ordering.'
//   },
//   {
//     value: 'modern',
//     label: 'Modern',
//     subtitle: 'Skills-first structure',
//     description: 'Highlights strengths and projects early for fresher profiles.'
//   },
//   {
//     value: 'compact',
//     label: 'Compact',
//     subtitle: 'Short and concise',
//     description: 'Condensed layout for quick recruiter scanning.'
//   }
// ];
...
//   const role = cleanLine(form.targetRole) || cleanLine(form.headline) || 'Professional Profile';
//   const contactLine = [
...
//   lines.push(fullName);
//   lines.push(role);
//   lines.push(contactLine || 'Add email | mobile | location');
...
//       educationLines.push(`- ${segment}${year ? ` | ${year}` : ''}${score ? ` | Score: ${score}` : ''}`);
//     });
//   } else {
//     educationLines.push('- Add your education details.');
//   }
...
//   const sectionContentByKey = {
//     summary: [cleanLine(form.profileSummary) || cleanLine(form.careerObjective) || 'Add your profile summary and career objective.'],
//     skills: [combinedSkills.length > 0 ? combinedSkills.join(', ') : 'Add technical, soft and tools skills.'],
//     education: educationLines,
//     projects: projects.length > 0 ? projects.map((item) => `- ${item}`) : ['- Add project details.'],
//     internships: internships.length > 0 ? internships.map((item) => `- ${item}`) : ['- Add internship details.'],
//     experience: experiences.length > 0 ? experiences.map((item) => `- ${item}`) : ['- Add experience details.'],
//     certifications: certifications.length > 0 ? certifications.map((item) => `- ${item}`) : ['- Add certifications.'],
//     achievements: achievements.length > 0 ? achievements.map((item) => `- ${item}`) : ['- Add achievements.'],
//     languages: [languages.length > 0 ? languages.join(', ') : 'Add languages known.'],
//     preferences: preferences.length > 0 ? preferences.map((item) => `- ${item}`) : ['- Add your job preferences.']
//   };
...
//   const selectedSections = templateSections[template] || templateSections.classic;
//   selectedSections.forEach(([title, key]) => {
//     pushSection(title, sectionContentByKey[key] || []);
//   });
...
//   const [selectedResumeTemplate, setSelectedResumeTemplate] = useState('classic');
...
//   const [scrollToResumeBuilder, setScrollToResumeBuilder] = useState(false);
...
//     const shouldOpenResume = section === 'resume';
//     const shouldFocusBuilder = location.hash === '#resume-builder' || focus === 'resume-builder';

//     if (shouldOpenResume) {
//       setActiveProfileSection('resume');
//     }

//     if (shouldOpenResume && shouldFocusBuilder) {
//       setScrollToResumeBuilder(true);
//     }
//   }, [location.hash, location.search]);

//   useEffect(() => {
//     if (!scrollToResumeBuilder) return;
//     if (activeProfileSection !== 'resume') return;
//     if (!resumeBuilderRef.current) return;

//     const timer = window.setTimeout(() => {
//       resumeBuilderRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' });
//       setScrollToResumeBuilder(false);
//     }, 120);

//     return () => window.clearTimeout(timer);
//   }, [activeProfileSection, scrollToResumeBuilder]);
...
//   const syncSessionUser = useCallback((profile) => {
...
//       retiredEmployeeId: currentUser?.role === 'retired_employee'
//         ? generateRetiredEmployeeId({
//           name: profile.name || currentUser.name || '',
//           mobile: profile.mobile || currentUser.mobile || ''
//         })
//         : currentUser.retiredEmployeeId || '',
//       studentCandidateId: currentUser?.role === 'retired_employee'
//         ? currentUser.studentCandidateId || ''
...
//   const hydrateProfileState = useCallback((updated) => {
//     setForm(updated);
...
//     [generatedResumeText]
//   );
//   const selectedResumeTemplateMeta = useMemo(
//     () => resumeTemplateOptions.find((templateOption) => templateOption.value === selectedResumeTemplate) || resumeTemplateOptions[0],
...
//     () => (isRetiredUser ? currentUser?.retiredEmployeeId || '' : currentUser?.studentCandidateId || ''),
//     [currentUser, isRetiredUser]
//   );
//   const generatedProfileId = isRetiredUser
//     ? generateRetiredEmployeeId({ name: form.name || '', mobile: form.mobile || '' })
//     : generateStudentCandidateId({ name: form.name || '', mobile: form.mobile || '' });
//   const profileIdLabel = isRetiredUser ? 'Retired Employee ID' : 'Student ID';
//   const profileIdValue = sessionProfileId || generatedProfileId;
...
//     setSuccess(`${selectedResumeTemplateMeta.label} resume draft generated from your profile.`);
//   };
...
//               <p><FiAward size={17} /> <span>{profileIdLabel}: {profileIdValue}</span></p>
//             </div>
//           </div>
//         </div>
...
//           <div ref={resumeBuilderRef} id="resume-builder" className="full-row resume-builder-card">
...
//             </div>
//             <div className="resume-template-picker" role="radiogroup" aria-label="Resume template selection">
//               {resumeTemplateOptions.map((templateOption) => (
//                 <button
//                   key={templateOption.value}
//                   type="button"
//                   role="radio"
//                   aria-checked={selectedResumeTemplate === templateOption.value}
//                   className={`resume-template-card ${selectedResumeTemplate === templateOption.value ? 'is-selected' : ''}`}
//                   onClick={() => setSelectedResumeTemplate(templateOption.value)}
//                 >
//                   <strong>{templateOption.label}</strong>
//                   <span>{templateOption.subtitle}</span>
//                   <small>{templateOption.description}</small>
//                   <div className={`resume-template-preview resume-template-preview--${templateOption.value}`} aria-hidden="true">
//                     <div className="resume-template-preview__bar resume-template-preview__bar--strong" />
//                     <div className="resume-template-preview__bar" />
//                     {templateOption.value === 'classic' ? (
//                       <>
//                         <div className="resume-template-preview__split">
//                           <span />
//                           <span />
//                         </div>
//                         <div className="resume-template-preview__list">
//                           <i />
//                           <i />
//                           <i />
//                         </div>
...
//                       <>
//                         <div className="resume-template-preview__chips">
//                           <span />
//                           <span />
//                           <span />
//                         </div>
...
//                       </>
//                     ) : null}
//                     {templateOption.value === 'compact' ? (
//                       <div className="resume-template-preview__list resume-template-preview__list--dense">
//                         <i />
//                         <i />
//                         <i />
//                         <i />
...
//                 Build {selectedResumeTemplateMeta.label} Resume
//               </button>
//               <button
//                 type="button"
...
const numberOrDecimalOnly = (value = '') => String(value || '').replace(/[^0-9.]/g, '');

const phoneMaxDigits = 10;
const pincodeMaxDigits = 6;
...
const parseGeneratedResumeText = (value = '') => {
  ...
  while (index < lines.length) {
    ...
    while (index < lines.length) {
      ...
      if (!current.trim()) {
        index += 1;
        if (lines[index] && lines[index].trim() && /^[-_]{3,}$/.test(next)) break;
        continue;
      }
...
const StudentProfilePage = () => {
  ...
  const isFieldLocked = !isEditMode || loading || saving || autoSaving || avatarSaving || resumeBuilderSaving;

  useEffect(() => {
    if (!isEditingName || isFieldLocked) return;
    nameInputRef.current?.focus();
    nameInputRef.current?.select();
  }, [isEditingName, isFieldLocked]);
  ...
  useEffect(() => {
    ...
    const handleOutsideClick = (event) => {
      if (avatarPopoverRef.current && !avatarPopoverRef.current.contains(event.target)) {
  ...
  useEffect(() => {
    ...
    if (shouldOpenResume) setActiveProfileSection('resume');
    if (shouldOpenResume && shouldFocusBuilder) setScrollToResumeBuilder(true);
  }, [location.hash, location.search]);
  ...
  const updateField = (key, value) => {
    ...
    if (key === 'mobile') nextValue = digitsOnly(value).slice(0, phoneMaxDigits);
    if (key === 'currentPincode' || key === 'permanentPincode') nextValue = digitsOnly(value).slice(0, pincodeMaxDigits);
    if (key === 'expectedSalary' || key === 'preferredSalaryMin' || key === 'preferredSalaryMax' || key === 'noticePeriodDays') {
  ...
  const updateEducationDraft = (key, value) => {
    ...
    if (
      key === 'startYear'
      || key === 'endYear'
      || key === 'expectedCompletionYear'
      || key === 'backlogs'
    ) {
      nextValue = digitsOnly(value).slice(0, 4);
      if (key === 'backlogs') nextValue = digitsOnly(value).slice(0, 2);
    }

    if (key === 'marksValue' || key === 'maxCgpa') {
      nextValue = numberOrDecimalOnly(value);
    ...
    setEducationDraft((current) => ({ ...current, [key]: nextValue }));
  };
  ...
  const parsedProjects = useMemo(() => projectsInput.split('\n').map((item) => item.trim()).filter(Boolean), [projectsInput]);
  const parsedTechnicalSkills = useMemo(() => toCommaArray(technicalSkillsInput), [technicalSkillsInput]);
  const parsedSoftSkills = useMemo(() => toCommaArray(softSkillsInput), [softSkillsInput]);
  const parsedTools = useMemo(() => toCommaArray(toolsInput), [toolsInput]);
  const parsedInternships = useMemo(() => internshipsInput.split('\n').map((item) => item.trim()).filter(Boolean), [internshipsInput]);
  const parsedCertifications = useMemo(() => certificationsInput.split('\n').map((item) => item.trim()).filter(Boolean), [certificationsInput]);
  const parsedPreferredWorkLocations = useMemo(() => toCommaArray(form.preferredWorkLocation), [form.preferredWorkLocation]);
  const generatedResumeLineCount = useMemo(
    () => (generatedResumeText ? generatedResumeText.split('\n').filter((line) => String(line).trim()).length : 0),
    [generatedResumeText]
  );
  const generatedResumePreview = useMemo(() => parseGeneratedResumeText(generatedResumeText), [generatedResumeText]);
  const selectedResumeTemplateMeta = useMemo(
    () => resumeTemplateOptions.find((templateOption) => templateOption.value === selectedResumeTemplate) || resumeTemplateOptions[0],
    [selectedResumeTemplate]
  );
  ...
  const missingDetailsCount = useMemo(() => profileChecks.filter((item) => !item).length, [profileChecks]);
  const profileCompletion = useMemo(() => {
    const doneCount = profileChecks.filter(Boolean).length;
    return Math.round((doneCount / Math.max(profileChecks.length, 1)) * 100);
  }, [profileChecks]);
  ...
  const profileActions = useMemo(
    () => [
      { key: 'exam', label: 'Add competitive exam', boost: '6%', done: parsedCertifications.length > 0, icon: FiAward },
      { key: 'internship', label: 'Add internship', boost: '8%', done: parsedInternships.length > 0, icon: FiBarChart2 },
      { key: 'project', label: 'Add project', boost: '7%', done: parsedProjects.length > 0, icon: FiFolder }
    ],
    [parsedCertifications.length, parsedInternships.length, parsedProjects.length]
  );
  ...
  const handleAddPreferredWorkLocation = () => {
    if (isFieldLocked) return;
    const locationValue = cleanLine(preferredLocationDraft);
    const pincodeValue = digitsOnly(preferredPincodeDraft).slice(0, pincodeMaxDigits);
    if (!locationValue || !pincodeValue) return;

    const nextEntry = `${locationValue} (${pincodeValue})`;
    const nextList = Array.from(new Set([...toCommaArray(form.preferredWorkLocation), nextEntry]));
    updateField('preferredWorkLocation', nextList.join(', '));
    setPreferredLocationDraft('');
    setPreferredPincodeDraft('');
  ...
  const handleDeleteEducation = (index) => {
    if (isFieldLocked) return;
    setEducationEntries((current) => current.filter((_, itemIndex) => itemIndex !== index));
    if (editingEducationIndex === index) handleAddEducation();
    setTimeout(() => scheduleAutoSave(250), 0);
  };
  ...
  const handleRemoveAvatar = async () => {
    if (!isEditMode || avatarSaving || loading || saving || autoSaving || resumeBuilderSaving) return;
    ...
    try {
      ...
      const updatedProfile = await updateStudentAvatar('');
      const merged = { ...form, ...updatedProfile, avatarUrl: updatedProfile.avatarUrl || '' };
      hydrateProfileState(merged);
      setIsAvatarExpanded(false);
      ...
      setAvatarMessage(removeError.message || 'Failed to remove profile photo.');
    } finally {
      setAvatarSaving(false);
    }
  };
  ...
  const toggleEditMode = () => {
    ...
    setError('');
    setSuccess(nextEditMode ? 'Edit mode is ON. Now you can update details.' : 'Profile locked.');
  ...
  const handleSave = async (event, sectionKey = activeProfileSection) => {
    event.preventDefault();
    if (!isEditMode) {
      setError('Click Edit Profile first to make changes.');
      return;
    }
  ...
  const handleUseGeneratedResume = () => {
    if (isFieldLocked || !generatedResumeText) return;
    updateField('resumeText', generatedResumeText);
    setSuccess('Generated resume added to Resume Text.');
  };
  ...
  return (
    <div className="module-page module-page--student">
      ...
      >
        ...
        >
          {isEditMode ? (
            <><FiLock size={16} style={{ marginRight: 8 }} /> Lock Profile</>
          ) : (
            <><FiUnlock size={16} style={{ marginRight: 8 }} /> Edit Profile</>
          )}
        </button>
      </div>
      ...
      <section className="profile-overview-card">
        <div className="profile-overview-main">
          <div
            className="profile-overview-avatar-col"
            ref={avatarPopoverRef}
            style={{ position: 'relative' }}
          >
            <div
              className={`profile-overview-avatar-ring ${avatarSaving ? 'is-uploading' : ''}`}
              style={{ '--profile-progress': `${profileCompletion}%` }}
            >
              <div
                className={`profile-overview-avatar ${form.avatarUrl ? 'is-zoomable' : ''}`}
                role={form.avatarUrl ? 'button' : undefined}
                tabIndex={form.avatarUrl ? 0 : -1}
                onClick={() => {
                  if (form.avatarUrl) setIsAvatarExpanded((current) => !current);
                }}
                onKeyDown={(event) => {
                  if (!form.avatarUrl) return;
                  if (event.key === 'Enter' || event.key === ' ') {
              ...
              >
                {form.avatarUrl ? (
                  <img src={form.avatarUrl} alt={`${profileName} profile`} />
                ) : (
              ...
              <button
                ...
                onClick={(event) => {
                  event.stopPropagation();
                  if (!isFieldLocked) avatarInputRef.current?.click();
                }}
                aria-label="Upload profile photo"
                title={isFieldLocked ? 'Enable edit mode to upload' : avatarSaving ? 'Uploading...' : 'Upload Photo'}
                disabled={isFieldLocked || avatarSaving}
              >
                <FiCamera size={14} />
              </button>
            ...
            {isEditMode ? (
              ...
              >
                <button
                  ...
                  disabled={!form.avatarUrl || avatarSaving}
                  style={{
                    minWidth: '130px',
                    height: '38px',
                    borderRadius: '999px',
                    ...
                    background: '#ffffff',
                    color: form.avatarUrl ? '#dc2626' : '#94a3b8',
                    cursor: !form.avatarUrl || avatarSaving ? 'not-allowed' : 'pointer',
                    display: 'inline-flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    gap: '8px',
            ...
            {isAvatarExpanded && form.avatarUrl ? (
              <div
                style={{
                  position: 'absolute',
                  top: '50%',
          ...
          <div className="profile-overview-details">
            <div className="profile-overview-title-row">
              {isEditingName && isEditMode && !loading && !saving && !autoSaving && !avatarSaving && !resumeBuilderSaving ? (
                <input
                  ref={nameInputRef}
                  className="profile-overview-name-input"
                  value={form.name}
                  ...
                  placeholder="Enter your name"
                  disabled={!isEditMode || loading || saving || autoSaving || avatarSaving || resumeBuilderSaving}
                  readOnly={!isEditMode}
                />
              ) : (
                <h2>{profileName}</h2>
              ...
              <button
                ...
                className="profile-overview-name-edit-btn"
                onClick={() => {
                  if (!isEditMode || loading || saving || autoSaving || avatarSaving || resumeBuilderSaving) return;
                  setIsEditingName((current) => !current);
                }}
                aria-label={isEditingName ? 'Done editing name' : 'Edit name'}
                title={!isEditMode ? 'Unlock profile first' : isEditingName ? 'Done' : 'Edit name'}
                disabled={!isEditMode || loading || saving || autoSaving || avatarSaving || resumeBuilderSaving}
              >
                {isEditingName ? <FiCheck size={18} /> : <FiEdit2 size={18} />}
              </button>
        ...
        <aside className="profile-overview-actions">
          ...
          <p className="module-note" style={{ marginTop: '10px' }}>
            {isEditMode ? 'Edit mode is ON. Now you can change profile details.' : 'Profile is locked. Click Edit Profile to make changes.'}
          </p>
        </aside>
      </section>
        ...
        <form className="form-grid" onSubmit={(event) => handleSave(event, activeProfileSection)} onBlurCapture={handleFormBlurCapture}>
          ...
          {activeProfileSection === 'personal' ? (
            <>
              ...
              <label className="full-row">
                Gender
                <select value={form.gender} onChange={(event) => updateField('gender', event.target.value)} disabled={isFieldLocked}>
                  {genderOptions.map((option) => (
                    <option key={option.value || 'empty'} value={option.value}>{option.label}</option>
                  ))}
                </select>
              ...
              <label className="full-row">
                Caste
                <select value={form.caste} onChange={(event) => updateField('caste', event.target.value)} disabled={isFieldLocked}>
                  {casteOptions.map((option) => (
                    <option key={option.value || 'empty'} value={option.value}>{option.label}</option>
                  ))}
                </select>
              ...
              <label className="full-row">
                Religion
                <select value={form.religion} onChange={(event) => updateField('religion', event.target.value)} disabled={isFieldLocked}>
                  {religionOptions.map((option) => (
                    <option key={option.value || 'empty'} value={option.value}>{option.label}</option>
                  ))}
                </select>
                  ...
                  <div className="profile-list-add-row profile-location-add-row">
                    ...
                    <label className="profile-inline-field">
                      ...
                      <input
                        ...
                        maxLength={6}
                        value={preferredPincodeDraft}
                        onChange={(event) => setPreferredPincodeDraft(digitsOnly(event.target.value).slice(0, pincodeMaxDigits))}
                        placeholder="400001"
                        disabled={isFieldLocked}
                      />
                    </label>
                    <button type="button" className="btn-primary profile-list-add-btn" onClick={handleAddPreferredWorkLocation} disabled={isFieldLocked}>
                      Add Preferred Location
                    </button>
                  </div>
                </div>
          ...
          {activeProfileSection === 'education' ? (
            <>
              ...
              <div className="full-row education-summary-card">
                <p>Profile completeness from education: <strong>{educationCompletion}%</strong></p>
                <button type="button" className="btn-link" onClick={handleAddEducation} disabled={isFieldLocked}>Add Education</button>
              </div>
                ...
                <div className="full-row education-entry-list">
                  {educationEntries.map((entry, index) => (
                    <article key={`${entry.educationLevel}-${entry.courseName}-${index}`} className="education-entry-card">
                      ...
                      <div className="education-entry-actions">
                        <button type="button" className="btn-link" onClick={() => handleEditEducation(index)} disabled={isFieldLocked}>Edit</button>
                        <button type="button" className="btn-link btn-link--danger" onClick={() => handleDeleteEducation(index)} disabled={isFieldLocked}>Delete</button>
                      </div>
                    </article>
                  ))}
                </div>
              ...
              <div className="full-row education-editor-grid">
                <label>
                  Education Level
                  <select value={educationDraft.educationLevel} onChange={(event) => updateEducationDraft('educationLevel', event.target.value)} disabled={isFieldLocked}>
                    {educationLevelOptions.map((option) => (
                      <option key={option.value || 'empty'} value={option.value}>{option.label}</option>
                    ))}
                  </select>
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) => updateEducationDraft('endYear', event.target.value)}
                    placeholder="2026"
                    disabled={isFieldLocked || educationDraft.educationStatus === 'pursuing'}
                  />
                </label>

                <label>
                  Course Type
                  <select value={educationDraft.courseType} onChange={(event) => updateEducationDraft('courseType', event.target.value)} disabled={isFieldLocked}>
                    {courseTypeOptions.map((option) => (
                      <option key={option.value || 'empty'} value={option.value}>{option.label}</option>
                    ))}
                  </select>
                ...
                <label>
                  Education Status
                  <select value={educationDraft.educationStatus} onChange={(event) => updateEducationDraft('educationStatus', event.target.value)} disabled={isFieldLocked}>
                    {educationStatusOptions.map((option) => (
                      <option key={option.value} value={option.value}>{option.label}</option>
                    ))}
                  </select>
                ...
                <label>
                  Medium of Education (optional)
                  <select value={educationDraft.mediumOfEducation} onChange={(event) => updateEducationDraft('mediumOfEducation', event.target.value)} disabled={isFieldLocked}>
                    {mediumOptions.map((option) => (
                      <option key={option.value || 'empty'} value={option.value}>{option.label}</option>
                    ))}
                  </select>
              ...
              {educationDraftError ? <p className="form-error full-row">{educationDraftError}</p> : null}
              <div className="full-row education-editor-actions">
                <button type="button" className="btn-primary" onClick={handleSaveEducationEntry} disabled={isFieldLocked}>
                  {editingEducationIndex >= 0 ? 'Update Education' : 'Add Education'}
                </button>
                {editingEducationIndex >= 0 ? (
                  <button type="button" className="btn-link" onClick={handleAddEducation} disabled={isFieldLocked}>Cancel Edit</button>
                ) : null}
              </div>
            </>
          ) : null}
                ...
                <div className="profile-list-add-row">
                  <input
                    ...
                    onKeyDown={(event) => {
                      if (event.key === 'Enter') {
                        event.preventDefault();
                        addCommaListItem(technicalSkillDraft, setTechnicalSkillDraft, technicalSkillsInput, setTechnicalSkillsInput);
                      }
                    }}
                    placeholder="Add one technical skill"
                    disabled={isFieldLocked}
                  />
                  <button type="button" className="btn-primary profile-list-add-btn" onClick={() => addCommaListItem(technicalSkillDraft, setTechnicalSkillDraft, technicalSkillsInput, setTechnicalSkillsInput)} disabled={isFieldLocked}>
                    Add
                  </button>
                </div>
                {parsedTechnicalSkills.length > 0 ? (
                  <div className="student-chip-row profile-chip-row">
                    {parsedTechnicalSkills.map((item, index) => (
                      <span key={`${item}-${index}`} className="student-chip profile-edit-chip">
                        {item}
                        <button type="button" onClick={() => removeCommaListItem(index, technicalSkillsInput, setTechnicalSkillsInput)} aria-label={`Remove ${item}`} disabled={isFieldLocked}>
                          ×
                        </button>
                      </span>
                    ))}
                ...
                <div className="profile-list-add-row">
                  <input
                    ...
                    onKeyDown={(event) => {
                      if (event.key === 'Enter') {
                        event.preventDefault();
                        addCommaListItem(softSkillDraft, setSoftSkillDraft, softSkillsInput, setSoftSkillsInput);
                      }
                    }}
                    placeholder="Add one soft skill"
                    disabled={isFieldLocked}
                  />
                  <button type="button" className="btn-primary profile-list-add-btn" onClick={() => addCommaListItem(softSkillDraft, setSoftSkillDraft, softSkillsInput, setSoftSkillsInput)} disabled={isFieldLocked}>
                    Add
                  </button>
                </div>
                {parsedSoftSkills.length > 0 ? (
                  <div className="student-chip-row profile-chip-row">
                    {parsedSoftSkills.map((item, index) => (
                      <span key={`${item}-${index}`} className="student-chip profile-edit-chip">
                        {item}
                        <button type="button" onClick={() => removeCommaListItem(index, softSkillsInput, setSoftSkillsInput)} aria-label={`Remove ${item}`} disabled={isFieldLocked}>
                          ×
                        </button>
                      </span>
                    ))}
                ...
                <div className="profile-list-add-row">
                  <input
                    ...
                    onKeyDown={(event) => {
                      if (event.key === 'Enter') {
                        event.preventDefault();
                        addCommaListItem(toolsDraft, setToolsDraft, toolsInput, setToolsInput);
                      }
                    }}
                    placeholder="Add one tool/technology"
                    disabled={isFieldLocked}
                  />
                  <button type="button" className="btn-primary profile-list-add-btn" onClick={() => addCommaListItem(toolsDraft, setToolsDraft, toolsInput, setToolsInput)} disabled={isFieldLocked}>
                    Add
                  </button>
                </div>
                {parsedTools.length > 0 ? (
                  <div className="student-chip-row profile-chip-row">
                    {parsedTools.map((item, index) => (
                      <span key={`${item}-${index}`} className="student-chip profile-edit-chip">
                        {item}
                        <button type="button" onClick={() => removeCommaListItem(index, toolsInput, setToolsInput)} aria-label={`Remove ${item}`} disabled={isFieldLocked}>
                          ×
                        </button>
                      </span>
                    ))}
              ...
              <div className="full-row profile-input-card">
                ...
                <div className="profile-list-add-row">
                  <input
                    ...
                    disabled={isFieldLocked}
                  />
                  <button type="button" className="btn-primary profile-list-add-btn" onClick={handleAddProject} disabled={isFieldLocked}>
                    Add
                  </button>
                </div>
                {parsedProjects.length > 0 ? (
                  <ul className="student-list profile-edit-list">
                    {parsedProjects.map((item, index) => (
                      <li key={`${item}-${index}`}>
                        <span>{item}</span>
                        <button type="button" className="btn-link btn-link--danger" onClick={() => handleRemoveProject(index)} disabled={isFieldLocked}>
                          Remove
                        </button>
                      </li>
                    ))}
          ...
          {activeProfileSection === 'resume' ? (
            <>
              ...
              <label>
                Resume URL
                <input value={form.resumeUrl} onChange={(event) => updateField('resumeUrl', event.target.value)} placeholder="https://example.com/resume.pdf" disabled={isFieldLocked} />
              </label>

              <label>
                Portfolio URL
                <input value={form.portfolioUrl} onChange={(event) => updateField('portfolioUrl', event.target.value)} placeholder="https://portfolio.example" disabled={isFieldLocked} />
              </label>

              <label>
                GitHub URL
                <input value={form.githubUrl} onChange={(event) => updateField('githubUrl', event.target.value)} placeholder="https://github.com/username" disabled={isFieldLocked} />
              </label>

              <label>
                LinkedIn URL
                <input value={form.linkedinUrl} onChange={(event) => updateField('linkedinUrl', event.target.value)} placeholder="https://linkedin.com/in/username" disabled={isFieldLocked} />
              </label>

              <label>
                Facebook URL
                <input value={form.facebookUrl} onChange={(event) => updateField('facebookUrl', event.target.value)} placeholder="https://facebook.com/username" disabled={isFieldLocked} />
              </label>

              <label>
                Instagram URL
                <input value={form.instagramUrl} onChange={(event) => updateField('instagramUrl', event.target.value)} placeholder="https://instagram.com/username" disabled={isFieldLocked} />
              </label>

              <label>
                Eimager ID
                <input value={form.eimagerId} onChange={(event) => updateField('eimagerId', event.target.value)} placeholder="Enter your Eimager ID" disabled={isFieldLocked} />
              </label>
              ...
              <div ref={resumeBuilderRef} id="resume-builder" className="full-row resume-builder-card">
                ...
                <div className="resume-template-picker" role="radiogroup" aria-label="Resume template selection">
                  {resumeTemplateOptions.map((templateOption) => (
                    <button
                      ...
                      aria-checked={selectedResumeTemplate === templateOption.value}
                      className={`resume-template-card ${selectedResumeTemplate === templateOption.value ? 'is-selected' : ''}`}
                      onClick={() => !isFieldLocked && setSelectedResumeTemplate(templateOption.value)}
                      disabled={isFieldLocked}
                    >
                      <strong>{templateOption.label}</strong>
                      <span>{templateOption.subtitle}</span>
                ...
                <div className="resume-builder-actions">
                  <button type="button" className="btn-primary resume-builder-btn" onClick={handleBuildResumeFromProfile} disabled={isFieldLocked || resumeBuilderSaving}>
                    Build {selectedResumeTemplateMeta.label} Resume
                  </button>
                  <button type="button" className="resume-builder-btn resume-builder-btn--ghost" onClick={handleUseGeneratedResume} disabled={isFieldLocked || !generatedResumeText || resumeBuilderSaving}>
                    Use in Resume Text
                  </button>
                  <button type="button" className="btn-primary resume-builder-btn" onClick={handleSaveGeneratedResumeToProfile} disabled={isFieldLocked || !generatedResumeText || resumeBuilderSaving}>
                    {resumeBuilderSaving ? 'Saving...' : 'Save to Profile'}
                  </button>
                  <button type="button" className="resume-builder-btn resume-builder-btn--ghost" onClick={handleDownloadGeneratedResume} disabled={!generatedResumeText || resumeBuilderSaving}>
                    Download .txt
                  </button>
                </div>
                ...
                {generatedResumeText ? (
                  ...
                  <p className="module-note">Click ,Build Classic Resume, or selected template button to preview your generated resume.</p>
                )}
              </div>
              ...
              <label>
                Expected Salary
                <input type="number" inputMode="numeric" value={form.expectedSalary} onChange={(event) => updateField('expectedSalary', event.target.value)} disabled={isFieldLocked} />
              </label>

              <label>
                Preferred Job Type
                <select value={form.preferredJobType} onChange={(event) => updateField('preferredJobType', event.target.value)} disabled={isFieldLocked}>
                  {preferredJobTypeOptions.map((option) => (
                    <option key={option.value || 'empty'} value={option.value}>{option.label}</option>
                  ))}
                </select>
              ...
              <label>
                Availability to Join
                <input value={form.availabilityToJoin} onChange={(event) => updateField('availabilityToJoin', event.target.value)} placeholder="Immediate / 15 days / 30 days" disabled={isFieldLocked} />
              </label>

              <label>
                Willing to Relocate
                <select value={String(form.willingToRelocate)} onChange={(event) => updateField('willingToRelocate', event.target.value)} disabled={isFieldLocked}>
                  <option value="">Select</option>
                  <option value="true">Yes</option>
                  <option value="false">No</option>
                </select>
              ...
              <label>
                Preferred Salary Min
                <input type="number" inputMode="numeric" value={form.preferredSalaryMin} onChange={(event) => updateField('preferredSalaryMin', event.target.value)} disabled={isFieldLocked} />
              </label>

              <label>
                Preferred Salary Max
                <input type="number" inputMode="numeric" value={form.preferredSalaryMax} onChange={(event) => updateField('preferredSalaryMax', event.target.value)} disabled={isFieldLocked} />
              </label>

              <label>
                Notice Period (days)
                <input type="number" inputMode="numeric" value={form.noticePeriodDays} onChange={(event) => updateField('noticePeriodDays', event.target.value)} disabled={isFieldLocked} />
              </label>
            </>
          ) : null}
          ...
          <div className="full-row profile-save-row">
            <button type="submit" className="btn-primary profile-save-btn" disabled={saving || !isEditMode}>
              {saving
                ? 'Saving...'
                : !isEditMode
                  ? 'Enable Edit Profile to Save'
                  : `Save ${profileFormSections.find((section) => section.key === activeProfileSection)?.label || 'Section'}`}
            </button>
          </div>
        </form>
