...
import { canAccessRole, getDashboardPathByRole, setAuthSession } from '../../../utils/auth';
import { findManagedAccountByEmail, updateManagedAccountLogin } from '../../../utils/managedUsers';
import {
...
const isManagedAccountAllowedForRoute = (role, redirectTo) => {
  ...
  if (redirectTo.startsWith('/portal/support')) return canAccessRole(role, 'support');
  if (redirectTo.startsWith('/portal/sales')) return canAccessRole(role, 'sales');
  if (redirectTo.startsWith('/portal/dataentry')) return canAccessRole(role, 'dataentry');
  if (redirectTo.startsWith('/portal/accounts')) return canAccessRole(role, 'accounts');
  if (redirectTo.startsWith('/portal/admin')) return canAccessRole(role, 'admin');
  if (redirectTo.startsWith('/portal/super-admin')) return canAccessRole(role, 'super_admin');
...
const LoginPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (tryManagedAccountLogin({ email: form.email, password: form.password, navigate, redirectTo, setError })) {
      return;
    }
    ...
    <div className="auth-page auth-page--login">
      <div className="auth-login-shell">
        ...
        <section className="auth-panel auth-panel--login">
          <h1>{portalLabel}</h1>
          <p>Sign in to continue to this dashboard.</p>
