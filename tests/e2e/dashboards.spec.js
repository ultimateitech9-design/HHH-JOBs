...
test('accounts dashboard smoke', async ({ page }) => {
  await openLogin(page);
...
test('data entry dashboard smoke', async ({ page }) => {
  await openLogin(page);
  await page.getByRole('button', { name: /demo data entry/i }).click();
...
test('super admin dashboard smoke', async ({ page }) => {
  await openLogin(page);
  await page.getByRole('button', { name: /demo super admin/i }).click();
  ...
  await expect(page.getByRole('button', { name: /create admin id/i })).toBeVisible();
...
test('sales order details link opens selected order', async ({ browser }) => {
  ...
  await expect(page.getByText('SO-2101', { exact: true })).toBeVisible();
  await expect(page.getByText('Metro Build Infra', { exact: true })).toBeVisible();
  } finally {
    await context.close();
  }
});
